...
from MCTS_UCB_Alphaz import MCTS
...
class AlphaZero:
    ...
    def selfPlay(self):
        ...
        while True:
            current_state = self.game.change_perspective(state, player)
            action_probs = self.mcts.search(current_state)
            
            memory.append((current_state, action_probs, player))
            ...
            if is_terminal:
                returnMemory = []
                for hist_current_state, hist_action_probs, hist_player in memory:
