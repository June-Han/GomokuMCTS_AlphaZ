...
class TreeSearch(object):
    ...
    def init_tree(uid, board_constructor, C):
        #initialize a root node with constructor `board_constructor` and store in var _maintained_tree with key `uid`
    ...
    @staticmethod
    def _explore(root, C):
        #Run one Monte Carlo simulation (with specified simu count) till whoever win and update simu_stats
        curr_node = root
        sim_path = []
        while True:
